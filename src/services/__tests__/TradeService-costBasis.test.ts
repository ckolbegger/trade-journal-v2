...
describe('TradeService - Cost Basis Integration', () => {
  ...
  it('[Service] should calculate cost basis after trade is persisted', async () => {
    ...
    const updated = await tradeService.addTrade('pos-123', {
      trade_type: 'buy',
      quantity: 100,
      price: 150.50,
      timestamp: new Date()
    ...
    // Cost basis should be calculated from updated position
    const costBasis = calculateCostBasis(updated.trades)
    expect(costBasis).toBe(150.50)
